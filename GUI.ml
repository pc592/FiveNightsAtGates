...
let update_disp oldIm newIm window =
  (* process click and call state? *)
  if oldIm = newIm then () else
  let image = Sdlloader.load_image ("Rooms/" ^ newIm) in
  ...
  Sdlvideo.blit_surface ~dst_rect:position_of_image ~src:image ~dst:window ();
  Sdlvideo.blit_surface ~dst_rect:position_of_text ~src:text ~dst:window ();
  Sdlvideo.blit_surface ~dst_rect:position_of_map ~src:map ~dst:window ();
